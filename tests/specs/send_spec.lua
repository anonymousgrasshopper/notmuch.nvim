...
local function with_compose_env(fn)
  ...
  local ok, err = pcall(fn, state, send, config)
...
return {
  {
    ...
    run = function()
      with_compose_env(function(_, send, config)
  ...
  },
  {
    ...
    run = function()
      with_compose_env(function(state, send, config)
